    ...
    .action(async (options) => {
      try {
        ...
        if (options.provider) {
          ...
          if (activeProvider) {
            results.activeProvider = await validateSpecificProvider(
              activeProvider.id,
            );
          }
        }
...
async function validateSpecificProvider(providerId: string): Promise<any> {
  ...
  try {
    ...
    return {
      isValid: false,
      errors: [
        `验证 Provider 时出错: ${error instanceof Error ? error.message : "未知错误"}`,
      ],
      providerId: provider.id,
      providerName: provider.name,
    };
...
function displayValidationResults(results: any): void {
  ...
  if (results.config) {
    ...
    console.log(
      chalk.gray(`   总 Providers: ${results.config.totalProviders}`),
    );
    console.log(
      chalk.gray(
        `   活跃 Provider: ${results.config.hasActiveProvider ? "是" : "否"}`,
      ),
    );
    console.log(
      chalk.gray(`   系统平台: ${results.config.systemInfo.platform}`),
    );
    console.log();
  }
  ...
  if (results.providers) {
    ...
    console.log(
      chalk.gray(
        `总计: ${results.providers.total}, 有效: ${chalk.green(results.providers.valid)}, 无效: ${chalk.red(results.providers.invalid)}\n`,
      ),
    );
    ...
    results.providers.results.forEach((result: any, index: number) => {
      ...
      console.log(
        `${icon} ${index + 1}. ${result.providerName} (${result.providerId}) - ${status}`,
      );
