      ...
      try {
        ...
        if (providerId) {
          ...
          if (!provider) {
            ...
            console.log(chalk.blue('💡 使用 ') + chalk.cyan('llmctl list') + chalk.blue(' 查看可用的 Providers'))
            process.exit(1)
          }
        ...
        const isActive = configManager.getActiveProvider()?.id === targetProviderId
        ...
        if (isActive) {
          console.log(chalk.yellow('⚠️  已删除的 Provider 是当前使用的，请选择新的 Provider'))
          console.log(chalk.blue('💡 使用 ') + chalk.cyan('llmctl use') + chalk.blue(' 选择新的 Provider'))
        }
        ...
        if (remainingProviders.length > 0 && isActive) {
          ...
          if (selectNew) {
            ...
            console.log(chalk.green(`🎯 已选择 "${newProvider.name}" 作为当前 Provider`))
          }
        }

      } catch (error) {
        console.error(chalk.red('❌ 删除 Provider 失败:'), error instanceof Error ? error.message : '未知错误')
        process.exit(1)
      }
    })
}
...
async function selectProviderToRemove(providers: any[]): Promise<string> {
  ...
  const choices = providers.map(provider => {
    ...
    const description = provider.description ? ` - ${provider.description}` : ''
