    ...
    .action(async (providerId, options) => {
      try {
        ...
        if (providers.length === 0) {
          ...
          console.log(chalk.blue('💡 使用 ') + chalk.cyan('llmctl add') + chalk.blue(' 添加一个新的 Provider'))
          return
        }
        ...
        console.log(chalk.green(`✅ 已选择 "${selectedProvider.name}" 作为当前 Provider`))
        ...
        console.error(chalk.red('❌ 选择 Provider 失败:'), error instanceof Error ? error.message : '未知错误')
        process.exit(1)
      }
    })
}
