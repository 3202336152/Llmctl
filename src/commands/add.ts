      ...
      try {
        ...
        if (options.template) {
          ...
          if (!template) {
            ...
            console.log(chalk.blue('💡 使用 ') + chalk.cyan('llmctl add --list-templates') + chalk.blue(' 查看可用模板'))
            process.exit(1)
          }
        ...
        console.log(chalk.blue('💡 使用 ') + chalk.cyan(`llmctl use ${provider.id}`) + chalk.blue(' 选择此 Provider'))
...
async function listProviderTemplates(): Promise<void> {
  ...
  templates.forEach((template, index) => {
    ...
    console.log(`   环境变量: ${chalk.yellow(Object.keys(template.envVars).join(', '))}`)
    console.log()
  })
}

async function selectProviderTemplate(templates: ProviderTemplate[]): Promise<ProviderTemplate> {
...
async function createProviderFromTemplate(template: ProviderTemplate): Promise<Provider> {
  ...
  if (answers.apiKey) {
    ...
    const apiKeyVar = envVarKeys.find(key => key.toLowerCase().includes('api_key') || key.toLowerCase().includes('token'))
    if (apiKeyVar) {
