...
export function createExportCommand(): Command {
  return new Command('export')
    ...
    .option('-f, --format <format>', '导出格式 (bash|powershell|cmd|json)', 'auto')
    ...
    .action(async (options) => {
      try {
        ...
        if (options.provider) {
          ...
          if (!provider) {
            console.error(chalk.red(`❌ Provider "${options.provider}" 不存在`))
            process.exit(1)
          }
        } else {
          ...
          if (!provider) {
            ...
            console.log(chalk.blue('   或使用 ') + chalk.cyan('llmctl export -p <provider-id>') + chalk.blue(' 指定特定的 Provider'))
            process.exit(1)
          }
        }
        ...
        if (options.dryRun) {
          console.log(chalk.blue(`🔍 预览 ${provider.name} 的环境变量 (${format} 格式):\n`))

          Object.entries(envVars).forEach(([key, value]) => {
            const maskedValue = key.toLowerCase().includes('key') || key.toLowerCase().includes('token')
              ? value.replace(/./g, '*').slice(0, 8) + '...'
              : value
            console.log(chalk.cyan(`${key}`) + chalk.gray('=') + chalk.yellow(maskedValue))
          })
        ...
        // 生成导出命令
        const exportCommand = EnvExporter.generateExportCommand(envVars, { format })
          ...
          if (process.stderr.isTTY) {
            ...
            switch (format) {
              case 'bash':
                ...
                console.error(chalk.gray('# 或: llmctl export | source /dev/stdin'))
                break
              case 'powershell':
                console.error(chalk.gray('# llmctl export --format powershell | Invoke-Expression'))
                break
              case 'cmd':
                console.error(chalk.gray('# llmctl export --format cmd > env.bat && call env.bat'))
                break
            }

            console.error(chalk.gray(`# 导出了 ${Object.keys(envVars).length} 个环境变量来自 ${provider.name}`))
          }
        }

      } catch (error) {
        console.error(chalk.red('❌ 导出环境变量失败:'), error instanceof Error ? error.message : '未知错误')
        process.exit(1)
      }
    })
}