...
export function createEditCommand(): Command {
  return new Command('edit')
    ...
    .action(async (providerId: string | undefined, options) => {
      try {
        const allProviders = configManager.getAllProviders()

        if (allProviders.length === 0) {
          console.log(chalk.yellow('📭 没有已配置的 Provider'))
          console.log(chalk.blue('💡 使用 ') + chalk.cyan('llmctl add') + chalk.blue(' 添加新的 Provider'))
          return
        }
        ...
        if (providerId) {
          ...
          if (!provider) {
            ...
            console.log(chalk.blue('💡 使用 ') + chalk.cyan('llmctl list') + chalk.blue(' 查看所有 Provider'))
            process.exit(1)
          }
        ...
        console.log(chalk.green(`✅ 成功修改 Provider: ${updatedProvider.name || targetProvider.name}`))
        ...
        if (currentProvider && currentProvider.id === targetProvider.id) {
          console.log(chalk.blue('💡 这是当前使用的 Provider，建议重新导出环境变量：'))
          console.log(chalk.cyan('llmctl export'))
        }

      } catch (error) {
        console.error(chalk.red('❌ 修改 Provider 失败:'), error instanceof Error ? error.message : '未知错误')
        process.exit(1)
      }
    })
}
...
async function editProvider(provider: Provider): Promise<Partial<Provider>> {
  ...
  const currentModel = provider.envVars?.ANTHROPIC_MODEL || provider.modelName || ''
  ...
  for (const fieldKey of fieldsToEdit) {
    ...
    if (fieldKey === 'apiKey') {
      ...
      const tokenKey = envVarKeys.find(key =>
        key.toLowerCase().includes('token') || key.toLowerCase().includes('api_key')
      )
      ...
      const { newValue } = await inquirer.prompt([
        {
          ...
          default: fieldKey === 'description' ? field.value : (field.value || undefined),
          validate: (input: string) => {
