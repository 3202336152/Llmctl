  ...
  return new Command('list')
    ...
    .action(async (options) => {
      try {
        ...
        console.log(chalk.bold(`📋 已配置的 LLM Providers (${providers.length}):\n`))
        ...
        providers.forEach((provider, index) => {
          ...
          if (options.verbose) {
            ...
            if (provider.maxTokens) {
              console.log(`   最大 Tokens: ${chalk.magenta(provider.maxTokens)}`)
            }
        ...
        console.error(chalk.red('❌ 列出 Providers 失败:'), error instanceof Error ? error.message : '未知错误')
        process.exit(1)
      }
    })
}