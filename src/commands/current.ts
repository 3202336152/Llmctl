    ...
    .action(async (options) => {
      try {
        ...
        if (options.json) {
          ...
          try {
            const validation =
              providerRegistry.validateProvider(activeProvider);
        ...
        if (
          ...
          if (activeProvider.maxTokens) {
            console.log(
              `   最大 Tokens: ${chalk.magenta(activeProvider.maxTokens)}`,
            );
          }

          if (activeProvider.temperature !== undefined) {
            console.log(
              `   温度: ${chalk.magenta(activeProvider.temperature)}`,
            );
          }
        }
        ...
        if (envVarCount > 0) {
          ...
          if (options.verbose) {
            Object.keys(activeProvider.envVars).forEach((key) => {
              ...
              console.log(
                `     ${chalk.cyan(key)}: ${chalk.yellow(maskedValue)}`,
              );
            });
          }
        ...
        try {
          ...
          console.log(
            chalk.red(
              `⚠️  无法验证配置: ${error instanceof Error ? error.message : "未知错误"}`,
            ),
          );
        }
      } catch (error) {
        console.error(
          chalk.red("❌ 获取当前 Provider 失败:"),
