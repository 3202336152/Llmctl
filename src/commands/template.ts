...
export function createTemplateCommand(): Command {
    ...
    templateCommand
        ...
        .action(async (options) => {
            ...
            console.log(chalk.bold(`📋 可用的 Provider 模板 (${templates.length}):\n`))
    ...
    templateCommand
        ...
        .action(async (templateId: string, options) => {
            ...
            if (!template) {
                ...
                console.log(chalk.blue('💡 使用 ') + chalk.cyan('llmctl template list') + chalk.blue(' 查看所有模板'))
                process.exit(1)
            }
...
async function createCustomTemplate(): Promise<ProviderTemplate> {
    ...
    while (addMore) {
        const {envKey, envValue, continue: shouldContinue} = await inquirer.prompt([
            {
                type: 'input',
                name: 'envKey',
                message: '请输入环境变量名称:',
