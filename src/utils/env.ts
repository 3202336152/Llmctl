...
export class EnvExporter {
  ...
  static async exportToCurrentShell(variables: Record<string, string>): Promise<EnvExportResult> {
    try {
  ...
  static validateEnvironmentVariables(variables: Record<string, string>): {
    ...
    Object.entries(variables).forEach(([key, value]) => {
      ...
      if (!/^[A-Z][A-Z0-9_]*$/.test(key)) {
        warnings.push(`环境变量名 "${key}" 不符合常见命名规范 (建议使用大写字母和下划线)`)
      }
  ...
  static async autoApplyEnvironmentVariables(
    ...
    try {
      ...
      if (format === 'cmd') {
        ...
        const commands = this.generateExportCommand(variables, { format: 'cmd' })
